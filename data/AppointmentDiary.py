...
import utils.datetimeHandler as datetimeHandler
...
class AppointmentDiary:
    ...
    def schedule_appt(self) -> str:
        ...
        # check conflicts
        conflicts = self.check_appointment_conflict(
    ...
    def reschedule_appt(self) -> str:
        ...
        # check
        appointment = self.check_appt_time_in_between(
            user_id, date, given_time)
        if not appointment:
            return "No appointment found for the specified date, and time."
        ...
        if self.check_appointment_conflict(user_id, new_date, new_time[0], new_time[1]):
            ...
            self.add_appointment_to_db(
                user_id, new_date, new_time[0], new_time[1], purpose)
    ...
    def delete_appt(self, user_id, date, start_time) -> None:
        ...
        self.db_handler.execute_query(
            query_cancel_appt, user_id, date, start_time)
    ...
    def check_appointment_conflict(self, user_id, date, start_time, end_time):
        ...
        conflicts = self.db_handler.fetch_all(
            query_conflict, user_id, date, start_time, start_time, end_time, end_time, start_time, end_time)
